...
def plot_3d_box(img, cam_to_img, ry, dimension, center, car_speed):
    ...
    # front side
    cv2.fillPoly(frame, np.array([[[box_3d[0]], [box_3d[1]], [box_3d[3]], [box_3d[2]]]], dtype=np.int32),
    ...
    edge_centers = []
    for idxs in [(0, 2, 6, 5), (1, 3, 5, 7), (1, 3, 2, 0), (5, 7, 6, 4), (1, 5, 4, 0), (3, 7, 6, 2)]:
        x = np.mean([box_3d[i][0] for i in idxs])
        y = np.mean([box_3d[i][1] for i in idxs])
        edge_centers.append((x, y))

    return CarDetection(edge_centers, car_speed)
